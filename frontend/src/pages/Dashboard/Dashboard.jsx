// Dashboard.jsx - Clean & Fixed Layout
...
const Dashboard = () => {
  ...
  const fetchPlayers = async () => {
    ...
    try {
      setLoading(true);
      let url = "http://localhost:5000/api/admin/players/pending";
      if (viewMode === "all") {
        url = "http://localhost:5000/api/admin/players/all";
      }
  ...
  const handleSelectAll = () => {
    if (selectAll) {
      ...
      setSelectedPlayers(currentPlayers.filter(p => p.status === "PENDING").map(p => p.id));
    }
    ...
    try {
      await Promise.all(
        selectedPlayers.map(id =>
          fetch(`http://localhost:5000/api/admin/player/${id}/status`, {
            method: "PATCH",
            headers: {
  ...
  const approvePlayer = async (id) => {
    try {
      await fetch(`http://localhost:5000/api/admin/player/${id}/status`, {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json"
  ...
  return (
    <div className="admin-container">
      <aside className="sidebar">
        <h1 className="logo">Crimson <span>Console</span></h1>
        <ul className="menu">
      ...
      <main className="main-content">
        ...
        {isPendingMode && selectedPlayers.length > 0 && (
          <div className="bulk-actions-bar">
            ...
            <div>
              <button onClick={() => bulkAction("APPROVED")} className="bulk-approve">Bulk Approve</button>
              <button onClick={() => bulkAction("REJECTED")} className="bulk-reject">Bulk Reject</button>
            </div>
          </div>
        )}
        ...
        <div className="results-info">
          Showing {indexOfFirstPlayer + 1}-{Math.min(indexOfLastPlayer, filteredPlayers.length)} of {filteredPlayers.length} player{filteredPlayers.length !== 1 ? "s" : ""}
        </div>
        ...
        <div className="table-wrapper">
          <table>
            <thead>
              <tr>
                {isPendingMode && <th><input type="checkbox" checked={selectAll} onChange={handleSelectAll} /></th>}
                <th>#</th>
                <th>Player</th>
                <th>Role</th>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={isPendingMode ? 8 : 7} className="no-data">Loading players...</td></tr>
              ) : currentPlayers.length === 0 ? (
                <tr><td colSpan={isPendingMode ? 8 : 7} className="no-data">No players match your filters</td></tr>
              ) : (
                  ...
                  <tr key={player.id} onClick={() => setSelectedPlayer(player)} style={{ cursor: "pointer" }}>
                    ...
                    <td>
                      <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                        <img
                          src={`http://localhost:5000${player.photoUrl}`}
                          alt={player.fullName}
                          style={{
                            width: "50px",
                            height: "50px",
                    ...
                    <td>
                      <span style={{
                        ...
                        borderRadius: "20px",
                        background:
                          player.status === "PENDING" ? "#7a0b0b" :
                          player.status === "APPROVED" ? "#0f5132" : "#8b0000",
                        color: "white",
                        fontSize: "13px",
                        fontWeight: "bold"
                      }}>
                    ...
                    {isPendingMode && (
                      <td onClick={(e) => e.stopPropagation()}>
                        <button className="approve-btn" onClick={() => approvePlayer(player.id)} disabled={player.status !== "PENDING"}>
                          Approve
                        </button>
                        <button className="reject-btn" onClick={() => rejectPlayer(player.id)} disabled={player.status !== "PENDING"}>
                          Reject
                        </button>
                      </td>
                    )}
          ...
          <div className="player-modal-overlay" onClick={() => setSelectedPlayer(null)}>
            <div className="player-modal-content" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="modal-header">
                <img
                  src={`http://localhost:5000${selectedPlayer.photoUrl}`}
              ...
              <div className="modal-body">
                <div className="modal-grid">
                  <div><strong>Date of Birth:</strong> {new Date(selectedPlayer.dob).toLocaleDateString()}</div>
                  <div><strong>Primary Phone:</strong> {selectedPlayer.primaryPhone}</div>
                  <div><strong>Alternate Phone:</strong> {selectedPlayer.alternatePhone || "-"}</div>
                  <div><strong>Blood Group:</strong> {selectedPlayer.bloodGroup}</div>
                  <div><strong>Aadhaar Number:</strong> {selectedPlayer.aadhaarNumber || "Not provided"}</div>
                  <div><strong>Primary Role:</strong> {selectedPlayer.primaryRole}</div>
                  <div><strong>Specialization:</strong> {selectedPlayer.battingProfile || selectedPlayer.bowlingStyle || selectedPlayer.allRounderType || "-"}</div>
                  <div><strong>Shirt Size:</strong> {selectedPlayer.shirtSize}</div>
                  <div><strong>Pant Size:</strong> {selectedPlayer.pantSize}</div>
                </div>
                ...
                <div className="modal-section">
                  ...
                  <img
                    src={`http://localhost:5000${selectedPlayer.aadhaarPhotoUrl}`}
                    alt="Aadhaar"
